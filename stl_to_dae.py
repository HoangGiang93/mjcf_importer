...
def stl_to_dae(file_path) -> None:
    for file in os.listdir(file_path):
        if file.endswith('.stl'):
            stl_mesh_path = os.path.join(file_path, file)
            dae_mesh_path = stl_mesh_path.replace("stl", "dae")
            bpy.ops.import_mesh.stl(filepath=stl_mesh_path)
            for object in bpy.data.objects:
                object.select_set(True)
                object.rotation_mode = 'XYZ'
                object.rotation_euler = (0, 0, 0)
                object.select_set(False)
            bpy.ops.wm.collada_export(filepath=dae_mesh_path)
            clear_data(bpy.data)
